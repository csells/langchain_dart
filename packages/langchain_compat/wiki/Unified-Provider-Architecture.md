...
- `Provider.allWith(Set<ProviderCaps>)` - Get providers with all specified capabilities
...
- **Static instances**: Each provider has a static instance (e.g., `Provider.openai`)
- **Name-based lookup**: `Provider.forName(String)` with case-insensitive matching
- **Alias support**: Providers can have alternative names (e.g., 'claude' → 'anthropic')
- **Discovery methods**: `Provider.all` lists all providers, `Provider.allWith()` filters by capabilities
...
sequenceDiagram
    ...
    rect rgb(200, 220, 240)
        ...
        Agent->>Provider: Provider.forName("openai")
...
Many providers use OpenAI's API format. The `OpenAIProvider` class can be instantiated with different configurations to support multiple providers (OpenRouter, Together, Lambda, etc.). See how `Provider.openrouter` and others are defined as configured OpenAIProvider instances.
