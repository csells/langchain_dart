...
- a ChatAgent no longer creates providers, they take existing instances by name or by ref.
  ...
  - 11 intrinsic providers, e.g. ChatProvider.google, ChatProvider.openai, etc.
  - providers and models manage API keys and base URLs
  - Agents use them by name or by ref, e.g. Agent('openai') or Agent.forProvider(ChatProvider.openai)
...
- Provider lookup by name or by using intrinsic providers
  ...
  - ChatProvider.name, e.g. ChatProvider.anthropic
  - ChatProvider.forName(name|alias), e.g. ChatProvider.forName('anthropic') or ChatProvider.forName('claude')
  - EmbeddingsProvider.name, e.g. ChatProvider.openai
  - EmebeddingsProvider.forName(name|alias), e.g. EmbeddingsProvider.forName['openai']
  - Agent.forProvider(ChatProvider.name), e.g. Agent.forProvider(MyCustomProvider()) or Agent.forProvider(ChatProvider.google)
