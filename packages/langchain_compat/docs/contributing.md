# Contributing to langchain_compat

We welcome contributions to langchain_compat! This guide will help you get started with contributing to the project.
...
2. Clone your fork locally:
   ```bash
   git clone https://github.com/YOUR_USERNAME/langchain_dart.git
   cd langchain_dart/packages/langchain_compat
   ```
...
void runProviderTest(
  ...
  for (final providerModel in providers) {
    test('$providerModel: $description', () async {
      final provider = Provider.forName(providerModel.split(':')[0]);
...
Thank you for contributing to langchain_compat!