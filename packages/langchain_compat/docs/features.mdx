...
- [x] Automatically check environment for API key if none is provided (not web
  compatible)
- [x] Streaming string output via `Agent.sendStream`
- [x] Multi-turn chat/message history support via the `history` parameter and
  the `ChatMessage` class (with roles and content types)
- [x] Typed output via `Agent.sendFor`
- [x] Define tools and their inputs/outputs
...
- [x] Embedding generation with `Agent.embedQuery` and `Agent.embedDocuments`
  with cosine similarity utilities
