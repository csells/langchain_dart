---
title: MCP (Model Context Protocol) Support
---

# MCP (Model Context Protocol) Support

Extend agents with external tools from MCP servers - remote APIs or local processes.

## Quick Start

```dart
// Connect to remote MCP server
final huggingFace = McpClient.remote(
  'huggingface',
  url: Uri.parse('https://hf.co/mcp'),
);

// Get tools from the server
final tools = await huggingFace.listTools();

// Use with your agent
final agent = Agent('google', tools: [...tools]);
final result = await agent.send('Who is hugging face?');
```

## Remote MCP Servers

Connect to hosted MCP endpoints:

```dart
final deepwiki = McpClient.remote(
  'deepwiki',
  url: Uri.parse('https://mcp.deepwiki.com/mcp'),
);

final tools = await deepwiki.listTools();
final agent = Agent('openai', tools: [...tools]);

await agent.send('What model providers does dartantic support?');
```

## Local MCP Servers

Connect to local processes:

```dart
final calculator = McpClient.local(
  'calculator',
  command: 'npx',
  args: ['-y', '@modelcontextprotocol/server-calculator'],
);

final tools = await calculator.listTools();
final agent = Agent('anthropic', tools: [...tools]);

await agent.send('What is 15 multiplied by 27?');
```

## Combining Multiple Sources

Mix local tools, MCP servers, and custom tools:

```dart
// Local tool
final timeTool = Tool(
  name: 'local_time',
  description: 'Get current time',
  onCall: (_) async => {'time': DateTime.now().toIso8601String()},
);

// MCP servers
final wiki = McpClient.remote('wiki', url: wikiUrl);
final hf = McpClient.remote('hf', url: hfUrl);

// Combine everything
final agent = Agent('google', tools: [
  timeTool,
  ...await wiki.listTools(),
  ...await hf.listTools(),
]);

await agent.send('What time is it and who is hugging face?');
```

## Available MCP Servers

Popular MCP servers to try:
- **HuggingFace** - `https://hf.co/mcp`
- **DeepWiki** - `https://mcp.deepwiki.com/mcp`
- **Local servers** - See [MCP servers list](https://github.com/modelcontextprotocol/servers)

## Examples

- [Complete MCP example](https://github.com/davidmigloz/langchain_dart/blob/main/packages/langchain_compat/example/bin/mcp_servers.dart)
- [MCP specification](https://spec.modelcontextprotocol.io/)

## Next Steps

- [Tool Calling](/tool-calling) - Create custom tools
- [Multi-Provider Conversations](/multi-provider-conversations) - Share tools across providers
