# Dartantic AI to Langchain Compat Migration Guide

This guide provides a comprehensive comparison between the old dartantic_ai package and the new langchain_compat (Dartantic 1.0) package, including detailed migration instructions with code snippets.
...
### Renamed Examples
- `multi_turn.dart` → `multi_turn_chat.dart`
...
## New Features in Langchain Compat
...
1. **Method Renames**: `run` → `send`, `runFor` → `sendFor`, `runStream` → `sendStream`
2. **Message Type**: `Message` → `ChatMessage`
3. **Provider Access**: Direct instantiation → Static registry (`Provider.openai`)
4. **Schema Definition**: Map-based → `JsonSchema` builder API
5. **File Handling**: `DataPart.file` → `DataPart.fromFile` with `XFile`
6. **New Features**: Embeddings, multi-provider chat, auto-retry, usage tracking
7. **Environment Variables**: System-only → System + programmatic
8. **Documentation**: Better organized with dedicated guides for each feature
